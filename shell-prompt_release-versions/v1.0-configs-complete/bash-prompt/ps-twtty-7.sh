...
# Changes made by Vasil Profirov:
# - Color Theme: edited the colors to be universaly visible and distinguishible for dark and light background themes
# (since these colors are overlayed on top of any terminal colors - Terminator - for example)

# NOTE: ps-twtty =>  Prompt String TermWide TeleTypeWriter
...
function twtty {
	# deprecated colors as no universal for light-dark background
	...
	# Color reversal/modification for the root user
	if [ "$(id -u)" == 0 ]; then
		local LIGHT_BLUE="\[\033[1;33m\]" # is-Yellow
		local YELLOW="\[\033[1;31m\]"     # is-Red
	fi
	...
	PS1="$TITLEBAR\
${CYAN}┌${LIGHT_BLUE}─(\
${YELLOW}\${my_D}${LIGHT_BLUE}, ${RED}Err ${PURPLE}\$?${LIGHT_BLUE}, ${PURPLE}\${my_TTY}\
${LIGHT_BLUE})─${CYAN}─\${my_FILL}${LIGHT_BLUE}─(\
${YELLOW}\${my_PWD}\
${LIGHT_BLUE})─${CYAN}─\
${NO_COLOUR}\n\
${CYAN}└${LIGHT_BLUE}─(\
${GREEN}\${USER}${LIGHT_BLUE}@${CYAN}\${HOSTNAME}\
${LIGHT_BLUE})${PURPLE}\$${NO_COLOUR} "
